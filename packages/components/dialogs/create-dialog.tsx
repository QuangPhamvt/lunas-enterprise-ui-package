  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex flex-col gap-0 border-none p-0" onInteractOutside={handleInteractOutside}>
        <DialogHeader className="flex-0 gap-2 border-b border-border-weak p-6 pb-4">
        ...
        <div className="bg-muted-muted size-full p-2">
          <div className="flex size-full flex-1 flex-col overflow-y-auto rounded-md bg-card p-4 shadow-card">
            <FormField
              name="name"
              validators={{
                onChange: ({ value }) => {
                ...
                onSubmit: ({ value }) => {
                  if (!value.trim().length) return `${nameLabel} is required`;
                  if (value.trim().length < minLength) return `${nameLabel} must be at least ${minLength} characters`;
                  if (maxLength && value.trim().length > maxLength) return `${nameLabel} must be at most ${maxLength} characters`;
                },
              }}
                ...
                <FieldGroup>
                  <Field orientation="vertical" className="gap-1" data-invalid={state.meta.isTouched && !state.meta.isValid}>
                    ...
                    <div className="flex flex-col space-y-0.5">
                      ...
                      <p
                        className={cn(
                          'text-end text-xs text-text-positive-weak',
                          maxLength !== undefined && state.value.length >= maxLength - 10 && maxLength >= 10 && 'text-warning',
                          maxLength !== undefined && state.value.length >= maxLength && 'text-danger'
                        )}
                      >
        ...
        <DialogFooter className="border-t border-border-weak p-6 pt-4">
          <Subscribe
            selector={state => ({ isSubmitting: state.isSubmitting, isDisabled: !state.isFormValid })}
